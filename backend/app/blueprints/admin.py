...
def dashboard():
    ...
    completed = Topic.query.filter_by(trang_thai=TopicStatus.DA_HOAN_THANH).count()
    failed = Topic.query.filter_by(trang_thai=TopicStatus.KHONG_DAT).count()
    in_progress = Topic.query.filter_by(trang_thai=TopicStatus.DANG_THUC_HIEN).count()
    proposed = Topic.query.filter_by(trang_thai=TopicStatus.CHO_XET_DUYET).count()
...
def periods():
    ...
    if request.method == "POST":
        ten_dot = request.form.get("ten_dot")
        nam_hoc = request.form.get("nam_hoc")
        cap_bac = request.form.get("cap_bac") or "Cấp Trường"
        thoi_gian_thong_bao = request.form.get("thoi_gian_thong_bao")
        thoi_gian_mo_dang_ky = request.form.get("thoi_gian_mo_dang_ky")
        han_nop_de_cuong = request.form.get("han_nop_de_cuong")
        han_nop_bao_cao = request.form.get("han_nop_bao_cao")
        if not (ten_dot and nam_hoc and cap_bac and thoi_gian_thong_bao and thoi_gian_mo_dang_ky and han_nop_de_cuong and han_nop_bao_cao):
            flash("Thiếu thông tin", "danger")
        else:
            thong_bao_dt = datetime.fromisoformat(thoi_gian_thong_bao)
            mo_dang_ky_dt = datetime.fromisoformat(thoi_gian_mo_dang_ky)
            nop_de_cuong_dt = datetime.fromisoformat(han_nop_de_cuong)
            nop_bao_cao_dt = datetime.fromisoformat(han_nop_bao_cao)
            if not (thong_bao_dt <= mo_dang_ky_dt <= nop_de_cuong_dt <= nop_bao_cao_dt):
                flash("Các mốc phải theo thứ tự: thông báo <= mở đăng ký <= nộp đề cương <= nộp báo cáo", "danger")
                return redirect(url_for("admin.periods"))
            p = RegistrationPeriod(
                ten_dot=ten_dot,
                nam_hoc=nam_hoc,
                cap_bac=cap_bac,
                thoi_gian_thong_bao=thong_bao_dt,
                thoi_gian_mo_dang_ky=mo_dang_ky_dt,
                han_nop_de_cuong=nop_de_cuong_dt,
                han_nop_bao_cao=nop_bao_cao_dt,
                trang_thai_dot=1,
            )
            p.sync_trang_thai_dot()
            db.session.add(p)
            db.session.commit()
            flash("Đã tạo năm học NCKH", "success")
            return redirect(url_for("admin.periods"))
...
@admin_bp.route("/admin_periods.html")
@login_required
def periods_alias():
    return redirect(url_for("admin.periods"))


@admin_bp.route("/topics/proposed")
@login_required
def topics_proposed():
    _ensure_admin()
    topics = (
        Topic.query
        .filter(Topic.trang_thai.in_([TopicStatus.CHO_XET_DUYET, TopicStatus.YEU_CAU_CHINH_SUA]))
...
def approve_topic(topic_id):
    ...
    topic.trang_thai = TopicStatus.DANG_THUC_HIEN
    topic.ly_do = None
    db.session.commit()
...
def require_revision(topic_id):
    ...
    topic.trang_thai = TopicStatus.YEU_CAU_CHINH_SUA
    topic.ly_do = reason
    db.session.commit()
    db.session.add(Notification(
...
def reject_topic(topic_id):
    ...
    topic.trang_thai = TopicStatus.BI_TU_CHOI
    topic.ly_do = reason
    db.session.commit()
    db.session.add(Notification(
...
def final_accept(topic_id):
    ...
    _ensure_admin()
    topic = Topic.query.get_or_404(topic_id)
    if topic.trang_thai not in (TopicStatus.CHO_NGHIEM_THU, TopicStatus.SUA_BAO_CAO):
        flash("Trạng thái hiện tại không cho phép nghiệm thu.", "warning")
        return redirect(request.referrer or url_for("admin.progress"))
    topic.trang_thai = TopicStatus.DA_HOAN_THANH
    topic.ly_do = None
    db.session.commit()
    db.session.add(Notification(
...
def topics():
    ...
    periods_data = [_serialize_period(period) for period in periods]
    return render_template("admin_topics.html", current_period_data=current_period_data, periods_data=periods_data)
